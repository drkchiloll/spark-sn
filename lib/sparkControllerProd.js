...
module.exports = function(sparkService) {
  ...
  sparkCntrler.archiveRoom = (req, res) => {
    ...
    var msg, roomId, htmlFile, txtFile, messages, roomName, members;
    if(reqData && reqData.text) {
      msg = (reqData.text).toLowerCase();
      roomId = reqData.roomId;
      if(msg === '/archive') {
        ...
        return Promise.all([
          ...
          sparkService.getRoomMembers(roomId)
           ...
           var archiveTxt = utils.memsAndMsgesText({
             roomName: roomName,
             messages: messages
           });
           console.log(archiveTxt);
