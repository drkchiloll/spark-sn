        ...
        if(msg.includes('archive')) {
          ...
          return sparkService.getRoomMessages(roomId).then(function(data) {
            ...
            messages.forEach(function(message) {
              messageBody += message.personEmail+'\n'+ message.text+'\n\n';
            });
            return db.findPoc(roomId).then(function(doc) {
              fileName = doc.room;
              fileName = fileName.replace(/\s/gi, '.');
              fileName += '.' + new Date().getTime() + '.txt';
              fs.writeFileSync(path.join(__dirname, '../public/files/'+ fileName), messageBody);
              // fs.closeSync(__dirname+'/public/files/'+ fileName);
              // Upload Static File into the Room
              return sparkService.sendMessage({
                roomId: payload.roomId,
                file: 'https://sparkint.proxy.wwtatc.com/fileupload/' + fileName
              }).then(function(data) {
                res.status(204).send({});
              })
            })
