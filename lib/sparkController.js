      ...
      if(msg) {
        if(msg.includes('archive')) {
          ...
          return sparkService.getRoomMessages(roomId).then(function(data) {
            ...
            var msgArchive = utils.constructHtml(messages);
            // console.log(utils.constructHtml(messages));
            return db.findPoc(roomId).then(function(doc) {
              htmlFile = doc.atcRequest;
              pdfFile = doc.atcRequest;
              htmlFile += '_' + new Date().getTime() + '.html';
              pdfFile += '_' + new Date().getTime() + '.pdf';
              var htmlFilePath = path.join(__dirname,'../public/files/'+htmlFile);
              var pdfFilePath = path.join(__dirname,'../public/files/'+pdfFile);
              fs.writeFileSync(filePath, msgArchive, 'utf8');
              ...
              pdf.create(html).toFile(pdfFilePath, function(err, res) {
                return sparkService.sendMessage({
                  ...
                  file: 'https://sparkint.proxy.wwtatc.com/fileupload/' + pdfFile
                }).then(function(data) {
                  res.status(204).send({});
                })
              })
