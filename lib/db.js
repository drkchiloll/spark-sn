...
module.exports = function(dbName) {
  ...
  */
  ...
  dbService.updRecords = (params) => {
    var update = {
      status: params.status || '',
      projectMgr: params.projectMgr || '',
      projectNumber: params.projectNumber || ''
    };
    if(!update.status) delete update.status;
    if(!update.projectMgr) delete update.projectMgr;
    if(!update.projectNumber) delete update.projectNumber;
    db.update({_id: params.id}, {
      $set: update
    }, {});
  }
