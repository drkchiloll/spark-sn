...
module.exports = (() => {
  ...
  /**
   * @param {Object} params
   * @param {String} params.filepath - Folder to Store PDF in
   * @param {String} params.fn - The PDF FileName
   * @param {String} params.html - HTML File to be Converted
   */
  helpers.htmlToPdf = (params) => {
    var cssPath =
      path.join(__dirname, '../node_modules/bootstrap/dist/css/bootstrap.css');
    var filePath = path.join(__dirname, `../public/files/${params.filepath}`);
    return new Promise((resolve, reject) => {
      pdf({
        cssPath: cssPath
      }).from(`${filePath}/document.html`).to(`${filePath}/${params.fn}.pdf`, () => {
        resolve('finished');
      })
    });
  };
